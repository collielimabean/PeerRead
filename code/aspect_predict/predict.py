...
def prepare_data(
  ...
  print('Total number of papers %d' %(np.sum([len(d) for _,d in list(data.items())])))
  print('Total number of reviews %d' %(np.sum([len(r['reviews']) for _,d in list(data.items()) for r in d ])))
